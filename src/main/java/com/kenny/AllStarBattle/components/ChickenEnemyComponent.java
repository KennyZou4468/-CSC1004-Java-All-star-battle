...
import com.kenny.AllStarBattle.Dir;
...
// 小鸡敌人组件，进行序列帧的播放
    ...
    public void onUpdate(double tpf) {
        ...
        switch (chickenmovedir){
         ...
         if(eggtimer.elapsed(Config.spawnegg)){
            egg();
         }
    }
    public ChickenEnemyComponent(){
// 拆分序列帧图
    ...
    public void egg(){
        ...
        List<Entity> entityList=entities.stream().filter(entity->
                ...
                entity.isType(Gametype.PLAYER2)).toList();
        if(entityList.isEmpty()){
            FXGL.spawn("egg",entity.getCenter());
        }
        eggtimer.capture();
    }
}
